#!/usr/bin/env tsx
/**
 * Script to automatically seed Chart of Accounts for all clients that don't have any
 * Run with: npx tsx scripts/seed-all-missing-coa.ts
 */
...
async function seedClientCoA(clientId: number, clientName: string) {
  console.log(`\nSeeding CoA for client ${clientId} (${clientName})...`);
...
// Identify and seed clients without accounts
async function seedAllMissingChartOfAccounts() {
  try {
    console.log('Identifying clients without Chart of Accounts...');
    ...
    console.log('\nSeeding missing Chart of Accounts for all clients...');
    ...
    for (const client of clientsWithoutAccounts) {
      const result = await seedClientCoA(client.id, client.name || 'Unnamed Client');
      results.push({
        clientId: client.id,
        name: client.name || '',
        ...result
    ...
    console.error('Error in seedAllMissingChartOfAccounts:', error);
  }
}
...
async function main() {
  try {
    await seedAllMissingChartOfAccounts();
  } catch (error) {
