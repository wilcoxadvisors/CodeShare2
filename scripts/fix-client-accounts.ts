#!/usr/bin/env tsx
/**
 * Script to identify clients without Chart of Accounts and seed them with standard template
 * Run with: npx tsx scripts/fix-client-accounts.ts
 */
...
// Identify clients without Chart of Accounts
async function identifyClientsWithoutAccounts() {
  try {
...
// Seed CoA for missing clients
async function fixMissingChartOfAccounts(clientsWithoutAccounts: any[]) {
  if (clientsWithoutAccounts.length === 0) {
    return;
  }
  
  const answer = await promptUser("\nDo you want to seed Chart of Accounts for these clients? (y/n): ");
...
async function main() {
  try {
    const clientsWithoutAccounts = await identifyClientsWithoutAccounts();
    if (clientsWithoutAccounts.length > 0) {
      await fixMissingChartOfAccounts(clientsWithoutAccounts);
    } else {
      rl.close();
    }
  } catch (error) {
