...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin-only access to entity_ids usage reports
  app.get("/api/admin/entity-ids-usage-report", isAuthenticated, hasRole("admin"), async (req: Request, res: Response) => {
    try {
      ...
      // Add metadata and insights for the frontend
      const enrichedReport = {
        ...report,
        metadata: {
          reportGeneratedAt: new Date(),
          daysIncluded: days,
          deprecationPhase: getDeprecationPhase(new Date())
        },
      ...
      console.error("Error generating entity_ids usage report:", error);
      res.status(500).json({ message: "Failed to generate usage report" });
    }
  });
  
  // Helper function to determine current deprecation phase
  function getDeprecationPhase(date: Date): string {
    ...
    if (date < phase1End) {
      return "Phase 1 - Dual Storage (Q1-Q2 2025)";
    } else if (date < phase2End) {
      return "Phase 2 - Junction Table Primary with Deprecation Notice (Q3 2025)";
    } else if (date < phase3End) {
      return "Phase 3 - Junction Table Only with entity_ids Hidden (Q4 2025)";
    } else {
      return "Phase 4 - Complete Removal (Q1 2026)";
    }
  }
  
  // Helper function to generate insights from the report
  function generateInsights(report: any): any {
    const insights = {
      summary: "No deprecated entity_ids usage detected.",
      recommendations: [] as string[],
      critical: false
    };
    
    if (report.directAccessCount > 0) {
      insights.summary = `Warning: ${report.directAccessCount} direct accesses to deprecated entity_ids detected.`;
      insights.critical = true;
      insights.recommendations.push("Eliminate direct entity_ids access in the following methods:");
      
      Object.entries(report.byMethod)
        .filter(([_, counts]: [string, any]) => counts.directAccessCount > 0)
        .sort((a, b) => b[1].directAccessCount - a[1].directAccessCount)
        .slice(0, 5) // Top 5 methods
        .forEach(([method, counts]: [string, any]) => {
          insights.recommendations.push(`- ${method}: ${counts.directAccessCount} occurrences`);
        });
    }
    
    if (report.fallbackUsageCount > 0 && !insights.critical) {
      insights.summary = `${report.fallbackUsageCount} fallbacks to entity_ids detected. May indicate missing junction table data.`;
      insights.recommendations.push("Ensure all consolidation groups have proper junction table entries.");
    }
