...
async function editEntity() {
  ...
  try {
    ...
    try {
      ...
      // Try to extract JSON from HTML response (if we got a HTML page)
      if (responseText.includes('<pre>')) {
        ...
        throw new Error(`Response is not valid JSON and does not contain extractable JSON`);
      }
    }
...
async function editEntityWithEmptyFields() {
  ...
  try {
    ...
    // We need to handle different response formats
    let problematicResponseText = await getResponse.text();
    console.log(`Raw entity response (problematic test): ${problematicResponseText.substring(0, 100)}...`);
    
    let entityData;
    try {
      entityData = JSON.parse(problematicResponseText);
      // Check if data is nested inside a data property
      if (entityData.data) {
        console.log(`Entity response has nested data structure`);
        entityData = entityData.data;
      ...
      console.error(`Failed to parse entity response as JSON: ${e.message}`);
      // Try to extract JSON from HTML response (if we got a HTML page)
      if (problematicResponseText.includes('<pre>')) {
        const jsonMatch = problematicResponseText.match(/<pre>(.*?)<\/pre>/s);
        if (jsonMatch && jsonMatch[1]) {
          try {
            entityData = JSON.parse(jsonMatch[1]);
            console.log(`Successfully extracted JSON from HTML response`);
    ...
    const updatedResponseText = await putResponse.text();
    console.log(`Problematic update response body:`, updatedResponseText);
